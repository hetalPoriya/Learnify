...
import 'package:learnify/apiService/api_services.dart';
...
class PaytmConfig {
  ...
  final String _url =
      'https://learnifyinfo.com/api/v1/${AppStrings.paytmChecksum}';
  ...
  Future<void> generateTxnToken(double amount, String orderId) async {
    ...
    try {
      ...
      Map<String, dynamic> txnToken = response.data;
      log('Response ${response.data}');
      // Map<String, dynamic> txt = jsonDecode(txnToken);
      // log('Response $txt');
      if (txnToken['Data']['body']['resultInfo']['resultStatus'] == 'S') {
        // Fluttertoast.showToast(
        //     msg: txt['Data']['body']['resultInfo']['resultMsg']);
        await initiateTransaction(
            orderId, amount, txnToken['Data']['body']['txnToken'], callBackUrl);
      } else {
        Fluttertoast.showToast(
            msg: txnToken['Data']['body']['resultInfo']['resultMsg']);
      }
    } catch (e) {
      if (kDebugMode) {
  ...
  Future<void> initiateTransaction(String orderId, double amount,
    ...
    try {
      var response = AllInOneSdk.startTransaction(mid, orderId,
          amount.toStringAsFixed(2), txnToken, callBackUrl, true, true);
